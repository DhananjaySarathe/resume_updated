...
export default function Experience() {
  return (
    <section id="experience" className="mx-auto w-full max-w-[1440px] px-margin-sm py-20 md:px-margin lg:px-margin-lg">
      <SectionHeader icon={History} eyebrow="Experience" title="Where I&apos;ve worked" className="mb-14" />
        ...
        {experience.map((job) => {
          ...
          return (
            <li key={job.role} className="relative" data-reveal>
              <div className="absolute -left-[31px] top-1 flex h-5 w-5 items-center justify-center rounded-pill bg-surface-container-lowest md:-left-[41px]">
                <span className={`h-3 w-3 rounded-pill ${nodeColor[job.accent]}`}>
                  {job.current && <span className="block h-full w-full animate-ping rounded-pill bg-primary opacity-60" />}
                </span>
              </div>
                ...
                <div className="mb-4 flex flex-col justify-between gap-2 sm:flex-row sm:items-start">
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {job.current && (
                        <span className="rounded-pill bg-primary-container/20 px-2.5 py-0.5 text-label-sm font-semibold uppercase tracking-wider text-primary">
                          Current
                        </span>
                      )}
                    </div>
                    <p className={`mt-0.5 text-body-md ${companyColor[job.accent]}`}>{job.company}</p>
                  </div>
                  <span className="shrink-0 text-label-md uppercase tracking-wider text-on-surface-variant">{job.period}</span>
                </div>

                {job.summary && <p className="text-body-md text-on-surface-variant">{job.summary}</p>}
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                    {job.highlights.map((h) => {
                      ...
                      return (
                        <div
                          key={h.title}
                          className="rounded-lg bg-surface-container/60 p-4 transition-colors duration-300 hover:bg-surface-container"
                        >
